...
(define (handle-messages univ wrld m)
  (cond 
    ...
    [(and (equal? (current_state univ) 'passed) 
          (equal? m 'passed))
     (let* ((calculated_score (calc-score (current_board univ)))
            (final_score (list
                          (+ (car (current_killed univ)) (cdr calculated_score));Punkte für weiß addieren
                          (+ (cadr (current_killed univ)) (car calculated_score))))) ;Punkte für schwarz addieren
       (if (< (car final_score) (cadr final_score))  ;;Schwarz hat gewonnen
           (if (equal? (get-color univ (iworld-name wrld)) -1) ;;Aktive Welt ist schwarz
               (make-bundle (list
                             (current_worlds univ)
                             'result
                             (current_board univ) (current_color univ) final_score)
                            (list (make-mail (world1 univ) (list 'won (current_board univ) final_score  'passed))
                                  (make-mail (world2 univ) (list 'lost (current_board univ) final_score  'passed)))
                            '())
               ...
               (make-bundle (list
                             ...
                             'result
                             (current_board univ) (current_color univ) final_score)
                            (list (make-mail (world1 univ) (list 'lost (current_board univ) final_score 'passed))
                                  (make-mail (world2 univ) (list 'won (current_board univ) final_score 'passed)))
                            '()))
           ;;Weiß hat gewonnen
